...
print('Running script n to successful run')
...
with open('transformed_data/n_to_next_run.csv', newline='') as csvfile:
    reader = csv.DictReader(csvfile)
    with open('transformed_data/n_to_successful_run.csv', 'w', newline='')  as output_file:

        current_row = next(reader)

        current_row['n_to_successful_run'] = 0
        ...
        while current_row:
            ...
            n_to_successful_run = 0
            for row in reversed(one_file_results):
                n_to_successful_run += 1

                row['n_to_successful_run'] = n_to_successful_run
                rev_results.append(row)
                if row['EventType'] == 'Run.Program' and row['would_compile'] == 'True':
                    n_to_successful_run = 0
