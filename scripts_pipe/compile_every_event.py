'''
This script attempts to compile every event by re-creating each student file,
mutating it with every event,
and compiling the re-created file.

Assumptions:
- The events have previously been sorted so that all events from a particular subject
  are grouped together. Within a subject group, events are correctly placed in chronological order 
  (with proper index-based resolution of timestamp equality during sorting)
'''
...
print('Running script compile_every_event')
...
with open(data_path, newline='') as csvfile:
    ...
    with open('transformed_data/compile_every_event.csv', 'w', newline='')  as output_file:
        ...
        for row in reader:
            ...
            #     status_message = f'{fail_count} out of {len(file_is_broken)} files are broken: {100 * fail_count / len(file_is_broken)} %'
