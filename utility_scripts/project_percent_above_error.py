...
print(f'The length of the input data is {len(initial_data)}')
...
percentage_above_error = (would_not_compile.groupby(['user_id', 'project_id', 'standardized_project', 'task'])
   .agg({
       'edit_above_or_at_error_int': ['mean', 'count']
    }))

# percentage_above_error.reset_index()
print(f'The length of the output (aggregated) data is {len(percentage_above_error)}')
percentage_above_error.columns = percentage_above_error.columns.map('_'.join)
percentage_above_error.to_csv('./transformed_data/project_percent_above_error.csv')
